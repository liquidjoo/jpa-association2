...
class EntityLoaderTest {
    ...
    static void beforeAll() throws SQLException {
        ...
        jdbcTemplate.execute(CreateQueryBuilder.INSTANCE.generateQuery(new PersistentClass(TestEntity.class)));
    ...
    void find_쿼리를_실행한다() {
        ...
        // when
        TestEntity actual = entityLoader.find(new PersistentClass<>(TestEntity.class), 1L);
    ...
    void eager_lazy_join_모두_포함된_entity를_반환한다() {
        ...
        jdbcTemplate.execute("insert into eager_order_items (id, orders_id, product, quantity) values (1, 1, '라면', 3);");
        jdbcTemplate.execute("insert into eager_order_items (id, orders_id, product, quantity) values (2, 1, '김치', 2);");
        jdbcTemplate.execute("insert into lazy_order_items (id, orders_id, product, quantity) values (1, 1, '라면', 3);");
        jdbcTemplate.execute("insert into lazy_order_items (id, orders_id, product, quantity) values (2, 1, '김치', 2);");
        jdbcTemplate.execute("insert into lazy_order_items (id, orders_id, product, quantity) values (3, 1, '바나나', 4);");
...
//                () -> assertThat(actual.lazyOrderItems).hasSize(3)
    ...
    void findAll_쿼리를_실행한다() {
        ...
        // when
        List<TestEntity> actual = entityLoader.findAll(new PersistentClass<>(TestEntity.class));
